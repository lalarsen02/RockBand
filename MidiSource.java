...
public final class MidiSource {
    ...
    private static void print(MidiMessage message) {
        if (message instanceof ShortMessage) {
            ...
            if (shortMessage.getCommand()
                    != 240) { // some MIDI controllers continousouly output a 240
        ...
        public void send(MidiMessage message, long timeStamp) {
            ...
            if (playSynth)
                if (message instanceof ShortMessage) {
                    ShortMessage shortMessage = (ShortMessage) message;
                    if (shortMessage.getCommand() == ShortMessage.NOTE_ON)
                        channels[shortMessage.getChannel()]
                                .noteOn(shortMessage.getData1(), shortMessage.getData2());
                    else if (shortMessage.getCommand() == ShortMessage.NOTE_OFF) {
                        channels[shortMessage.getChannel()]
                                .noteOff(shortMessage.getData1(), shortMessage.getData2());
                    }
                }
        }
